...
export function PriceListManager({
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const path = `price-list/${orgId}/${Date.now()}.${ext}`;
    ...
    await addPriceListFlyer(publicUrl, maxOrder + 1);
    setUploading(false);
    ...
    <section className="space-y-4">
      <div>
        ...
        <p className="text-xs text-stone-400 mt-0.5">
          Se muestran como imágenes a pantalla completa en la página pública. Máx. 5 MB por imagen.
        </p>
      </div>
      ...
      {items.length > 0 ? (
        <div className="bg-white rounded-xl border border-stone-200 divide-y divide-stone-100">
          {items.map((item, index) => (
            <div key={item.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <p className="flex-1 text-sm text-stone-400">Flyer {index + 1}</p>
        ...
        <div className="bg-white rounded-xl border border-dashed border-stone-200 py-10 text-center text-stone-400 text-sm">
          No hay flyers de precios todavía.
        </div>
      )}

      <div className="bg-white rounded-xl border border-stone-200 p-4 space-y-3">
        <p className="text-xs font-medium text-stone-600">Agregar flyer</p>
        <label
