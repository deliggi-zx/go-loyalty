...
interface DomusChatPanelProps {
  slug: string;
  orgId: string;
  whatsappNumber: string | null;
  open: boolean;
  onClose: () => void;
  /** Título de la barra del panel. Domus usa "Asistente Domus"; Kapusta,
   *  "Asistente" a secas (su marca no es Domus). */
  title?: string;
  /** Clases de posición del panel (fixed + coordenadas). Por defecto queda
   *  apilado sobre el launcher de DomusChatWidget (abajo a la derecha). El
   ...
   *  botón flotante y el resto del sitio. */
  accentColor?: string;
}

// Panel de conversación en sí — sin el botón lanzador. Lo usan tanto
// DomusChatWidget (launcher fijo, Domus) como el dock flotante de Kapusta
// (launcher arrastrable). Toda la lógica de chat vive acá una sola vez.
export function DomusChatPanel({
  slug,
  orgId,
  whatsappNumber,
  open,
  ...
  // (hoy Domus/Kapusta no tienen whatsapp_number cargado) — sin número,
  // apunta a la home con el ancla del formulario de consultas ya
  // existente (ver id="consultas" en page.tsx), en vez de un link muerto.
  ...
  return (
    <div
      className={`${positionClassName} h-[28rem] bg-white rounded-2xl shadow-xl border border-stone-200 flex flex-col overflow-hidden transition-all duration-200 ${
...
// Launcher fijo abajo a la derecha + panel. Es la forma que usa Domus
// (home). Kapusta no usa este componente: monta DomusChatPanel desde su
// dock flotante con un botón arrastrable propio (ver kapusta-floating-dock.tsx).
